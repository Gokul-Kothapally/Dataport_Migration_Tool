...
ImportPersonnelAsync(string apiToken, JObject PersonnelJson, string moduleNumber, Guid? historyId = null, string baseFolderPath = "", Guid agencyKey = default)
        {
            if (PersonnelJson == null || PersonnelJson.Count == 0)
            ...
            try
            {
                ...
                var json = JsonConvert.SerializeObject(PersonnelJson);
                var content = new StringContent(json, Encoding.UTF8, "application/json");

                var requestModel = new CommonRequestModel
                {
                    Endpoint = "Document/Personnel",
