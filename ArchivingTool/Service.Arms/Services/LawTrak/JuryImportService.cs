...
ImportJuryAsync(string apiToken, JObject JuryJson, string moduleNumber, Guid? historyId = null, string baseFolderPath = "", Guid agencyKey = default)
        {
            if (JuryJson == null || JuryJson.Count == 0)
            ...
            try
            {
                ...
                var json = JsonConvert.SerializeObject(JuryJson);
                var content = new StringContent(json, Encoding.UTF8, "application/json");

                var requestModel = new CommonRequestModel
                {
                    Endpoint = "Document/Jury",
