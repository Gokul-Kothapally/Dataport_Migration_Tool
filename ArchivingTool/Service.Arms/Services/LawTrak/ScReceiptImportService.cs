...
{
    public class ScReceiptImportService
    {
        private readonly BlobUploadHelper blobUploadHelper;
        private readonly HttpClientService _httpClientService;

        public ScReceiptImportService()
        {
...
ImportScReceiptAsync(string apiToken, JObject ScReceiptJson, string moduleNumber, Guid? historyId = null, string baseFolderPath = "", Guid agencyKey = default)
        {
            if (ScReceiptJson == null || ScReceiptJson.Count == 0)
                return (false, null, "No data");

            var attachmentService = new AttachmentService(_httpClientService, blobUploadHelper);
            var module = "ScReceipt";
            ...
            try
            {
                ...
                var json = JsonConvert.SerializeObject(ScReceiptJson);
                var content = new StringContent(json, Encoding.UTF8, "application/json");

                var requestModel = new CommonRequestModel
                {
                    Endpoint = "Document/ScReceipts",
